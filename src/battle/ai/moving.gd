...
onready var paths = [
	"../../../left_path", "../../../slight_left_path", "../../../stright_path",
...
func _ready() -> void:
	...
	next_curve.add_point(Vector2.ZERO, Vector2.ZERO, Vector2(1,1))
	var x = rng.randi_range(0,4)
	print(x)
	next_curve.add_point(Vector2.ZERO + get_node(paths[x]).position, Vector2(1,1))
...
func physics(scorpion) -> String:
	...
	if path_follow.unit_offset == 1:
		...
		rng.randomize()
		var next_curve = Curve2D.new()
		next_curve.add_point(scorpion.position, Vector2(10,0), Vector2(10,0))
		var x = rng.randi_range(0,4)
		print(x)
